...
class _OnDemandUserPageState extends State<OnDemandUserPage> with AutomaticKeepAliveClientMixin {
  ...
  final fieldsIncompleteSnackBar = SnackBar(
      content: Text('Fields incomplete')
  );
          ...
          bottomNavigationBar: UserButton(
            ...
            onClick: () {
              if (allFieldsFilled()) {
                ...
                Scaffold.of(context).showSnackBar(fieldsIncompleteSnackBar);
              }
            },
          ),
        );
  ...
  bool allFieldsFilled() {
    if (onDemandInputs.hospital.text.isNotEmpty && onDemandInputs.department.text.isNotEmpty && onDemandInputs.genderType != null) {
      if ((onDemandInputs.isBookingForOthers == false) || (onDemandInputs.isBookingForOthers == true && onDemandInputs.patientName.text.isNotEmpty)) {
        return true;
      } else {
        return false;
      }
  ...
  Future<void> getOnDemandStatus() async {
    ...
    OnDemandStatus _onDemandStatus = await OnDemandServices().getOnDemandStatus(isSLI: false, headerToken: _authToken);
    setState(() {
